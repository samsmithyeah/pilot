...
/**
 * Find the most recently built TapsmithAgent xctestrun in Xcode's DerivedData.
 *
 * The DerivedData hash changes whenever Xcode regenerates it, so hardcoding
 * the path is brittle. This walks all TapsmithAgent-* dirs and returns the
...
export function findLatestXctestrun() {
  const pattern = join(
    homedir(),
    "Library/Developer/Xcode/DerivedData/TapsmithAgent-*/Build/Products/*.xctestrun",
  )
  // Exclude `.patched.xctestrun` files left behind by previous Tapsmith runs.
  ...
  if (matches.length === 0) {
    throw new Error(
      `No xctestrun found at ${pattern}\n\n` +
        ...
        `    -project TapsmithAgent.xcodeproj -scheme TapsmithAgentUITests \\\n` +
        `    -destination 'platform=iOS Simulator,name=iPhone 17'\n\n` +
        `Or set TAPSMITH_IOS_XCTESTRUN explicitly (see e2e/.env.example).`,
    )
  }
