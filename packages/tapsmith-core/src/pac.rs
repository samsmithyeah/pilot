...
//! `http://<host_ip>:<port>/tapsmith.pac` on every Wi-Fi join (and, iOS version
//! permitting, periodically thereafter). The function decides per-host whether
//! traffic should route through the Tapsmith proxy or go DIRECT.
//!
//! Why PAC instead of `ProxyType=Manual`:
//!
//!   1. **Automatic fallback when Tapsmith is down.** With `PROXY …; DIRECT` as
...
//!      permanently and the phone works with or without Tapsmith running —
...
//!     Wi-Fi before `tapsmith test` runs, the fetch fails and iOS falls back to
...
///
/// Security note: patterns are user-provided via tapsmith.config.ts. We escape
...
pub fn generate_pac_script(proxy_host: &str, proxy_port: u16, network_hosts: &[String]) -> String {
    ...
    if network_hosts.is_empty() {
        return format!(
            "// Tapsmith auto-generated PAC (catch-all — no trace.networkHosts configured)\n\
    ...
    format!(
        "// Tapsmith auto-generated PAC — filters to trace.networkHosts allowlist\n\
         function FindProxyForURL(url, host) {{\n\
...
mod tests {
    ...
    fn generated_pac_starts_with_comment() {
        let pac = generate_pac_script("192.168.1.5", 9037, &[]);
        assert!(pac.starts_with("// Tapsmith"));
    }
}
