...
//! Instead, Tapsmith generates a per-device configuration profile (`.mobileconfig`)
//! that installs:
//!   1. A Wi-Fi entry with manual HTTP proxy pointing at the host's LAN IP +
//!      a deterministic per-UDID port
//!   2. The Tapsmith MITM CA root certificate for HTTPS trust
//!
//! The user installs the profile on the device once (AirDrop / email), then
//! `tapsmith test` against a physical device binds the MITM proxy on
...
pub fn generate_mobileconfig(inputs: &MobileconfigInputs) -> Result<Vec<u8>> {
    ...
    wifi.insert(
        ...
        Value::String(format!("Tapsmith proxy for {}", inputs.device_name)),
    );
    wifi.insert(
        "PayloadDescription".into(),
        Value::String(
            "Routes Wi-Fi traffic through Tapsmith's MITM proxy for network capture.".into(),
        ),
    );
    ...
    //
    //   1. Automatic fallback to DIRECT when the Tapsmith daemon isn't
    ...
    // `http://<host>:<port>/tapsmith.pac`. iOS fetches it on Wi-Fi join
    ...
    // where the phone joins Wi-Fi before `tapsmith test` starts the daemon.
    ...
    let ca_der = pem_to_der(&inputs.ca_pem)
        .context("Failed to decode Tapsmith CA PEM before embedding in mobileconfig")?;
    ...
    cert.insert(
        "PayloadDisplayName".into(),
        Value::String("Tapsmith MITM CA".into()),
    );
    cert.insert(
        "PayloadDescription".into(),
        Value::String(
            "Root certificate for Tapsmith's on-path MITM proxy. Must be trusted in \
    ...
    cert.insert(
        ...
        Value::String("tapsmith-ca.pem".into()),
    );
    ...
    top.insert(
        "PayloadDisplayName".into(),
        Value::String(format!("Tapsmith Network Capture — {}", inputs.device_name)),
    );
    top.insert(
        "PayloadDescription".into(),
        Value::String(
            "Installs a Wi-Fi HTTP proxy and trusts the Tapsmith MITM CA so Tapsmith tests can \
...
mod tests {
    ...
    fn generate_mobileconfig_contains_key_fields() {
        ...
        assert!(xml.contains("dev.tapsmith.networkcapture.UDID-FAKE-0001"));
    }
