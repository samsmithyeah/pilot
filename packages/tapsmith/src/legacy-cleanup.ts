 ...
 * Tapsmith versions. Called from both the parallel dispatcher and the
 * single-worker CLI fast path so the notice shows regardless of which code
 * path executes the user's `tapsmith test` invocation.
 *
 ...
 * approach (`tapsmith setup-proxy`). Tapsmith no longer uses `networksetup` for
 * iOS network capture — iOS traffic routing is now handled by the macOS
 * Network Extension redirector inside tapsmith-core. If this file is still
 ...
 */
const LEGACY_SUDOERS_FILE = '/etc/sudoers.d/zzz-tapsmith-networksetup';
let _legacySudoersNoticeShown = false;

/**
 * Print a deprecation notice (to stderr) if the legacy `zzz-tapsmith-
  ...
  try {
    if (fs.existsSync(LEGACY_SUDOERS_FILE)) {
      process.stderr.write(
        `${YELLOW}[tapsmith]${RESET} ${DIM}Legacy sudoers file ${LEGACY_SUDOERS_FILE} is no longer used by Tapsmith.\n` +
