 ...
 * generation time (`~/.tapsmith/devices/<udid>.meta.json`) and compare its
...
/** Sidecar shape as written by `tapsmith-core::physical_device_proxy`. */
...
export function sidecarPathForDevice(udid: string): string {
  return path.join(os.homedir(), '.tapsmith', 'devices', `${udid}.meta.json`);
}
