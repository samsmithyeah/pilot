...
import { APIRequestContext, TapsmithAPIResponse } from '../api-request.js';
...
describe('TapsmithAPIResponse', () => {
  it('constructs with correct properties', () => {
    const headers = new Headers({ 'content-type': 'text/plain' });
    const res = new TapsmithAPIResponse(200, 'OK', true, 'http://example.com', headers, Buffer.from('hello'));
    expect(res.status).toBe(200);
  ...
  it('dispose is callable', () => {
    const res = new TapsmithAPIResponse(200, 'OK', true, '', new Headers(), Buffer.alloc(0));
